...
func usage() {
	...
	fmt.Fprintln(out, "\nLet an operator into the console. One generated password per person;")
	fmt.Fprintln(out, "the label is the username and the password proves it. The first credential")
	fmt.Fprintln(out, "on a fresh database is automatically a manager. This is a shell job because")
	fmt.Fprintln(out, "the console cannot enrol its own first operator — that would need the login")
	fmt.Fprintln(out, "it does not have yet:")
	fmt.Fprintln(out, "  bykami -db … admin password add kasir-1")
	fmt.Fprintln(out, "  bykami -db … admin password list")
	fmt.Fprintln(out, "  bykami -db … admin password rm kasir-1")
	fmt.Fprintln(out, "  bykami -db … admin password manage kasir-1")
	fmt.Fprintln(out, "  bykami -db … admin password unmanage kasir-1")
