...
func usage() {
	...
	fmt.Fprintln(out, "the label is the username and the password proves it. This is a shell")
	fmt.Fprintln(out, "job because the console cannot enrol its own first operator — that would")
	fmt.Fprintln(out, "need the login it does not have yet:")
