...
localhost. It exists for one reason: `getUserMedia` refuses to run on an insecure
origin, so the capture flow cannot be tried on a phone or a tablet without real
HTTPS and a real hostname. A booth in a shop sets neither flag.

When `-public-host` is set, the booth requires a username+password login for
every route except the download gallery and the brand logo. The login page is
server-rendered with no JavaScript. If no account exists yet, the public surface
answers 503 with a plain message so the box stays reachable for account creation:

```
No booth account exists yet.
Create one with: bykami-agent access add <username>
```

Accounts are stored in the local SQLite database, hashed with PBKDF2-SHA256 at
600,000 iterations and a per-account random salt. Passwords are generated by the
CLI and printed once; there is no endpoint that mints them, because a credential
vending machine on the unauthenticated side would hand out the booth.

Requests arriving over localhost still need no login — demanding one there would
be theatre, since anything on that machine can read it.

The session cookie is `__Host-bykami-booth-session`: `HttpOnly`, `Secure`,
`SameSite=Strict`, a 12-hour expiry, and deliberately no CSRF token beside it.
`Strict` is what makes the token unnecessary: a POST from another site arrives
without the cookie, so the guard redirects it to the login before any handler
runs, and a foreign `Origin` is refused outright. A token would have needed a
delivery path into the kiosk UI, and the first cut of this shipped none — every
state-changing POST from a browser on the public host answered 403 while the
tests passed, because they called the test-only accessor instead of the page.
There is a `TestStateChangingPOSTIsAdmitted` for the admitted path and two
`Origin` tests for the refused one.

**`/g/…` is exempt, and has to be.** The session cookie is the *booth's* — it
opens `/api/capture` and `/api/print` — so handing it to a customer to collect
their photographs would hand them the booth. The download gallery carries its
own, much narrower secret instead; see below. The exemption matches the gallery
...
`booth_enabled` is true. After the role runs, somebody must create the first
account with `bykami-agent access add <username>` before the tunnelled hostname
is usable.
