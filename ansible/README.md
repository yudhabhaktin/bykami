...
- **Put secrets in the unit file.** `/etc/systemd/system` is world-readable and
  `ExecStart` is visible in `ps`. The frame-sync token goes in a mode-0600
  `EnvironmentFile`.
- **Start the booth without a public host but no account.** The agent now uses
  a username+password login instead of a URL token. After this release,
  somebody must run `bykami-agent access add <name>` on the box before
  `booth-test` is usable again. The role does not create accounts — a password
  generated by Ansible and written to a vars file would be no better than the
  old token.
